...
func (m *cloudflareStruct) add(domain string) cloudflare.Zone {
	zone, err := m.api.CreateZone(domain, false, cloudflare.Account{}, "full")
	if err != nil {
		_, fn, line, _ := runtime.Caller(0)
		panic(filepath.Base(fn) + ":" + strconv.Itoa(line-2) + " >> " + err.Error() + " >> " + fmtDebugStack(string(debug.Stack())))
	}
...
func (m *cloudflareDomainStruct) list() (res []cloudflareRecord) {
	records, err := m.api.DNSRecords(m.DomainID, cloudflare.DNSRecord{})
	if err != nil {
		_, fn, line, _ := runtime.Caller(0)
		panic(filepath.Base(fn) + ":" + strconv.Itoa(line-2) + " >> " + err.Error() + " >> " + fmtDebugStack(string(debug.Stack())))
	}
...
func (m *cloudflareDomainStruct) delete(name string, dtype string, value string) {
	// 虽然之后有log的代码, 但是在这个函数里面关闭log
	logLevel := lg.getLevel()
	lg.setLevel("")
	defer lg.setLevel(logLevel)

	dtype = strLower(dtype)
	if name == "@" {
		name = m.DomainName
	}
	lg.debug(name, dtype, value)
	for _, v := range m.list() {
		if name != "" && dtype != "" && value != "" {
			lg.trace("")
			if v.Name == name && v.Type == dtype && v.Data == value {
				lg.trace("")
				err := m.api.DeleteDNSRecord(m.DomainID, v.ID)
				if err != nil {
					_, fn, line, _ := runtime.Caller(0)
					panic(filepath.Base(fn) + ":" + strconv.Itoa(line-2) + " >> " + err.Error() + " >> " + fmtDebugStack(string(debug.Stack())))
				}
			...
			lg.trace("")
			lg.trace(v.Name, name, v.Type, dtype)
			...
			lg.trace("")
			lg.trace(v.Name, name)
			if v.Name == name {
				lg.trace("")
				lg.trace(m.DomainID, v.ID)
				err := m.api.DeleteDNSRecord(m.DomainID, v.ID)
				if err != nil {
					_, fn, line, _ := runtime.Caller(0)
					panic(filepath.Base(fn) + ":" + strconv.Itoa(line-2) + " >> " + err.Error() + " >> " + fmtDebugStack(string(debug.Stack())))
...
func (m *cloudflareDomainStruct) add(recordName string, recordType string, recordValue string, proxied ...bool) *cloudflare.DNSRecordResponse {
	...
	resp, err := m.api.CreateDNSRecord(m.DomainID, cloudflare.DNSRecord{
...
func (m *cloudflareDomainStruct) modify(recordName string, srcRecordType string, srcRecordValue string, dstRecordType string, dstRecordValue string) *cloudflare.DNSRecordResponse {
	srcRecordType = strLower(srcRecordType)
	dstRecordType = strLower(dstRecordType)
	if func() bool {
		for _, v := range m.list() {
			if v.Name == recordName && v.Type == srcRecordType && v.Data == srcRecordValue {
		...
		_, fn, line, _ := runtime.Caller(0)
		panic(filepath.Base(fn) + ":" + strconv.Itoa(line-1) + " >> 需要修改的记录未找到 >> " + fmtDebugStack(string(debug.Stack())))
	}

	return m.add(recordName, dstRecordType, dstRecordValue)
}

func (m *cloudflareDomainStruct) setProxied(domain string, proxied bool) {
	for _, v := range m.list() {
		//lg.trace(v.Name, domain, proxied)
		if v.Name == domain {
			if v.Proxiable == false {
				_, fn, line, _ := runtime.Caller(0)
				panic(filepath.Base(fn) + ":" + strconv.Itoa(line-1) + " >> 类型为" + v.Type + "的dns记录无法设置cdn代理加速 >> " + fmtDebugStack(string(debug.Stack())))
			} else {
				m.delete(v.Name, v.Type, v.Data)
				m.add(v.Name, v.Type, v.Data, proxied)
			}
		}
	}
}
...
func (m *xpathStruct) childHTML() string {
	return htmlquery.OutputHTML(m.doc, false)
}
...
func (m *tableStruct) addRow(row ...interface{}) {
	if len(row) != len(m.header) {
		_, fn, line, _ := runtime.Caller(0)
		panic(filepath.Base(fn) + ":" + strconv.Itoa(line-2) + " >> 添加的数据个数跟表头的个数对不上 >> " + fmtDebugStack(string(debug.Stack())))
	}
